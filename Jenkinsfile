...
pipeline {
  ...
  stages {
    ...
    stage('Mongo DB') {
      steps {
        script {
          container = docker.image("devops2022.azurecr.io/dropdrop:dbpush8").run("-p 27017:27017 -d -e buildNr=${tag} -e buildId=${BUILD_ID} -e nodeName=${NODE_NAME} -e nodeLabels=${NODE_LABELS}") 
          sh "docker ps"
