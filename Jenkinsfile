...
pipeline {
  ...
  stages {
    ...
    stage('Mongo DB') {
      steps {
        script {
          container = docker.image("devops2022.azurecr.io/dropdrop:dbpush8").run("-p 27017:27017 -d -e buildNr=${tag} -e buildId=${BUILD_ID} -e jobName=${JOB_NAME} -e buidlTag=${BUILD_TAG} -e nodeName=${NODE_NAME} -e nodeLabels=${NODE_LABELS} -e gitCommit=${GIT_COMMIT} -e gitPrevCommit=${GIT_PREVIOUS_COMMIT} -e gtiPrevSucCommit=${GIT_PREVIOUS_SUCCESSFUL_COMMIT} -e gitcommitName=${GIT_AUTHOR} ") 
          sh "docker ps"
